// 站点级脚本：注入顶部细导航 + 按配置渲染板块文章列表
// 图标库：全部为手绘线条构成 logo（viewBox 0 0 48 48，描边 currentColor，无现成图标）
(function () {
  ...
  var ICONS = {
    ...
    'policy-dark':    S + '<path d="M12 8 H32 L38 14 V40 H12 Z"/><path d="M32 8 V14 H38"/><path d="M16 18 H30"/><path d="M16 24 H34"/><path d="M16 30 H34"/><path d="M16 36 H27"/><circle cx="33" cy="33" r="2.5" fill="currentColor" fill-opacity=".2"/>' + E,
  ...
  if (!document.querySelector('.site-top')) {
    ...
    if (section) {
      ...
      var name = sec ? sec.title : section;
      crumb += '<span class="crumb"><a href="' + root + 'index.html">首页</a> &nbsp;/&nbsp; ' + name + '</span>';
    }
    top.innerHTML = crumb + '<span class="spacer"></span><a class="home" href="' + root + 'index.html">YTF 首页 ↗</a>';
    document.body.insertBefore(top, document.body.firstChild);
  }

  // 2) 板块文章列表渲染：图标 + 标题 + 关键字 + 简介
  ...
  if (listEl && section && window.SECTIONS) {
    ...
    if (sec && sec.articles && sec.articles.length) {
      ...
      sec.articles.forEach(function (a) {
        ...
        html += '<a class="art-card" href="' + href + '">'
              + '<div class="art-cover">' + icon + '</div>'
              + '<div class="art-body">'
              + '<h4>' + a.title + '</h4>'
              + (a.tag ? '<div class="kw-row"><span class="kw">' + a.tag + '</span></div>' : '')
              + '<div class="art-desc">' + (a.desc || '') + '</div>'
              + '<div class="meta"><span class="m-label">提交</span> ' + (a.date || '') + '</div>'
              + '</div></a>';
      });
